...
async function loadWebLLM(onProgress) {
  ...
  return CreateMLCEngine(WEBLLM_MODEL, {
    initProgressCallback: (p) => onProgress({ progress: Math.round(p.progress * 100), text: p.text || 'Downloading Gemma 2...' })
  })
}
...
async function loadTransformers(onProgress) {
  ...
  return pipeline('text-generation', FALLBACK_MODEL, {
    dtype: 'q4',
    progress_callback: (p) => { if (p.progress) onProgress({ progress: Math.round(p.progress), text: 'Downloading backup AI model...' }) }
  })
}
      ...
      try {
        ...
        if (gpu) {
          try {
            ...
            if (!cancelled) { engineRef.current = engine; tierRef.current = 'webllm'; setEngineTier('Gemma 2') }
          } catch (e) {
...
export const PROMPTS = {
  writingFeedback: `IMPORTANT: You must ALWAYS reply in English only. Do not use any other language under any circumstances.
...
Keep under 150 words. Always write in English.`,
  ...
  conversationTutor: `IMPORTANT: You must ALWAYS reply in English only. Do not use any other language under any circumstances.
