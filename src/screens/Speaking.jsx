...
function SpeakingTask({ task, onBack }) {
  ...
  const startRecording = useCallback(async () => {
    setRecordTime(0); setRecording(true)
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      const SR = window.SpeechRecognition || window.webkitSpeechRecognition
      const recognition = new SR(); recognitionRef.current = recognition
      recognition.continuous = true; recognition.interimResults = false; recognition.lang = 'en-US'
      let parts = []; recognition.onresult = e => { for (let i = e.resultIndex; i < e.results.length; i++) { if (e.results[i].isFinal) parts.push(e.results[i][0].transcript) }; setTranscript(parts.join(' ')) }; recognition.start()
    }
  ...
  return (
    <div>
      ...
      <div className="section-body" style={{ paddingTop: 12 }}>
        {phase === 'prep' && (
          <div>
            <div className="card card-accent-forest" style={{ background: 'var(--forest-pale)', marginBottom: 14 }}>
              <p className="bold text-small" style={{ color: 'var(--forest)', marginBottom: 8 }}>Speak about these points:</p>
              {task.prompts.map((p, i) => <p key={i} className="text-small" style={{ marginBottom: 4 }}>• {p}</p>)}
            </div>
            {prepTime === task.prep ? (<div style={{ textAlign: 'center', padding: '24px 0' }}><p className="text-muted text-small" style={{ marginBottom: 16 }}>You have {task.prep} seconds to prepare.</p><button className="btn btn-primary" onClick={startPrep}>Start preparation timer</button></div>) : (<div style={{ textAlign: 'center', padding: '24px 0' }}><div style={{ fontSize: '3rem', fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--forest)' }}>{prepTime}</div><p className="text-muted text-small">seconds to prepare</p></div>)}
          </div>
        )}
        {phase === 'record' && (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            <p className="bold" style={{ marginBottom: 8 }}>{task.topic}</p>
            <p className="text-small text-muted" style={{ marginBottom: 24 }}>Speak clearly and confidently.</p>
            <div style={{ marginBottom: 20 }}>
              <button className={`record-btn ${recording ? 'recording' : ''}`} onClick={recording ? stopRecording : startRecording}>{recording ? '⏹' : '🎤'}</button>
            </div>
            {recording && (<><div style={{ fontSize: '2rem', fontFamily: 'var(--font-serif)', fontWeight: 700, color: 'var(--danger)' }}>{recordTime}s</div><p className="text-small text-muted">Recording...</p></>)}
            {!recording && <p className="text-small text-muted">Tap 🎤 to start recording</p>}
          </div>
        )}
        {phase === 'transcribe' && (
          <div>
            <h3 style={{ marginBottom: 10 }}>What was captured</h3>
            {transcript ? <div className="passage-text"><p>{transcript}</p></div> : <p className="text-muted text-small">No transcript captured.</p>}
            <button className="btn btn-primary btn-full mt-16" onClick={analyseTranscript}>Get AI feedback →</button>
          </div>
        )}
        {phase === 'feedback' && (
          <div>
            <h3 style={{ marginBottom: 10 }}>AI Feedback</h3>
            {isThinking ? <div className="ai-thinking"><span className="spinner" /> Analysing...</div> : <div className="ai-feedback">{feedback}</div>}
            {!isThinking && (<div className="mt-16 flex-col gap-8"><button className="btn btn-primary btn-full" onClick={() => { setPhase('prep'); setPrepTime(task.prep); setTranscript(''); setFeedback('') }}>Try again</button><button className="btn btn-ghost btn-full" onClick={onBack}>← Choose another topic</button></div>)}
          </div>
        )}
      </div>
    </div>
